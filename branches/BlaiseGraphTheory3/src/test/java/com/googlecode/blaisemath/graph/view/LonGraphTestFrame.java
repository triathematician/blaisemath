...
import com.googlecode.blaisemath.graph.lon.PreferentialAttachmentLonGraphGenerator;
...
import com.googlecode.blaisemath.graph.view.LonGraphComponent;
import com.googlecode.blaisemath.graph.view.LonGraphListComponent;
 ...
 * @author ae3263
 */
public class LonGraphTestFrame extends javax.swing.JFrame {
    ...
    LonGraphListComponent multi;

    /** Creates new form TestPlaneVisometry */
    public LonGraphTestFrame() {
        EditorRegistration.registerEditors();
        initComponents();
        // BASIC ELEMENTS
        LonGraph<Integer> g = new PreferentialAttachmentLonGraphGenerator().generate(
                new PreferentialAttachmentParameters(new ExtendedGeneratorParameters(false, 10, 10), 100, new float[]{0,.95f,.05f}));
        plot.setLonGraph(g);
        plot.getGraphComponent().zoomToAll();

        multi = new LonGraphListComponent(plot.getLonGraphManager());
...
//        rollupPanel1.add("Visometry", new PropertySheet(plot.getGraphComponent().getVisometry()));
        ...
        rollupPanel1.add("Simultaneous Layout", PropertySheet.forBean(plot.getLonGraphManager().getLayoutAlgorithm().getParameters()));
    ...
    private void randomLBActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_randomLBActionPerformed
        ...
        plot.getLayoutManager().applyLayout(RandomBoxLayout.getInstance(), Collections.EMPTY_MAP, Collections.EMPTY_SET,
                new BoxLayoutParameters(new Rectangle2D.Double(-200, -200, 400, 400)));
    ...
    private void circleLBActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_circleLBActionPerformed
        ...
        plot.getLayoutManager().applyLayout(CircleLayout.getInstance(), Collections.EMPTY_MAP, Collections.EMPTY_SET, 
                new CircleLayoutParameters(200));
    ...
    private void timeEnergyIBActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_timeEnergyIBActionPerformed
        if (energyLayout == null)
            energyLayout = new SpringLayout(plot.getLayoutManager().getNodeLocationCopy());
        plot.getLonGraphManager().iterateLayout();
        updateEL = false;
    ...
    private void timeEnergyABActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_timeEnergyABActionPerformed
        if (energyLayout == null)
            energyLayout = new SpringLayout(plot.getLayoutManager().getNodeLocationCopy());
        plot.getLonGraphManager().startLayoutTask(10, 2);
    ...
    private void timeEnergySBActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_timeEnergySBActionPerformed
        plot.getLonGraphManager().stopLayoutTask();
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new LonGraphTestFrame().setVisible(true);
            }
        });
    }
    ...
    private LonGraphComponent plot;
