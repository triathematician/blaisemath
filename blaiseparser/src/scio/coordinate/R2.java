...
//    public void setY(double y){this.y = y;}
    ...
    /** Gives angle as value between 0 and 2pi */
    public double angle(){
        double result = Math.atan(y/x);
        if (x < 0) { return Math.PI + result; }
    ...
    // TESTS
    public boolean isInfinite() { return equals(INFINITY); }
    public boolean isZero() { return equals(ORIGIN); }
...
//    }
    ...
    public static double angle(R2 p){return angle(p.x, p.y);}
    ...
    public static R2 atInfiniteAngle(double angle) {
        return getPolar(Math.pow(10,10), angle);
    }

    /** Returns slope between two points. */
    public static java.lang.Double slope(R2 p1, R2 p2) { return (p2.y-p1.y)/(p2.x-p1.x); }

    /** Returns cross product of three points, by first subtracting vectors. */
    public static java.lang.Double crossProduct(R2 p1, R2 p2, R2 p3) {
        return p2.minus(p1).cross(p3.minus(p1));
    }

    /** Returns center of circle passing through three points.
     * Center is at infinity if points are colinear.
    ...
    public static R2 threePointCircleCenter(R2 p1, R2 p2, R2 p3) {
        ...
        if (p1.isInfinite()) {
            return twoPointInfinity(p2,p3);
        } else if (p2.isInfinite()) {
            return twoPointInfinity(p1,p3);
        } else if (p3.isInfinite()) {
            return twoPointInfinity(p1,p2);
        }
    ...
    /** Returns point infinitely far away from two others but equidistant from the two. */
    public static R2 twoPointInfinity(R2 p1, R2 p2) {
        if(p1.equals(p2)) { return R2.INFINITY; }
        return R2.atInfiniteAngle(angle(p2.minus(p1)) + Math.PI/2);
    }
}
