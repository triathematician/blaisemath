 ...
 * be invoking a method. Typical values used include "`EDT`" for the swing
 * event-dispatch thread, "`multiple`" where the method may be called from
 * multiple threads, or "`unknown`" where the thread calling the method is
 * not known. Other values can also be used. This annotation generally "propagates"
 * in two ways. If a method has an `InvokedFromThread` annotation, any
 ...
 * be widened, e.g. moving from `InvokedFromThread("my-thread")` to
 ...
 * within one of those methods might have the annotation `InvokedFromThread("multiple")`
 * or `InvokedFromThread("unknown")`.
 *
 * @author Elisha Peterson
 */
@Retention(RetentionPolicy.SOURCE)
@Target(AnnotationTarget.FUNCTION, AnnotationTarget.PROPERTY_GETTER, AnnotationTarget.PROPERTY_SETTER)
annotation class InvokedFromThread(
        /**
