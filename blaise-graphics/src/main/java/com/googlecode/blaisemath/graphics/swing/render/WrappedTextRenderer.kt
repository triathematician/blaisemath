...
class WrappedTextRenderer : TextRenderer() {
    ...
    override fun render(text: AnchoredText?, style: AttributeSet?, canvas: Graphics2D?) {
        ...
        for (t in lines) {
            TextRenderer.Companion.getInstance().render(t.getText(), t.getStyle(), canvas)
        }
    }
    ...
    override fun boundingBox(text: AnchoredText?, style: AttributeSet?, canvas: Graphics2D?): Rectangle2D? {
        ...
        for (t in lines) {
            val box: Rectangle2D = TextRenderer.Companion.getInstance().boundingBox(t.getText(), t.getStyle(), canvas)
     ...
     */
    fun computeLines(text: String?, style: AttributeSet?, textBounds: Shape?, insets: Insets?, canvas: Graphics2D?): Iterable<StyledText?>? {
        var canvas = canvas
        if (canvas == null) {
    ...
    private fun computeEllipseLines(text: String?, style: AttributeSet?, ell: Ellipse2D?, canvas: Graphics2D?): MutableList<StyledText?>? {
        ...
        val centeredStyle = AttributeSet.withParent(style).and(Styles.TEXT_ANCHOR, Anchor.CENTER)
