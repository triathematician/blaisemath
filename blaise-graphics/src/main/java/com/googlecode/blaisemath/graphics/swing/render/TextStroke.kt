...
internal class TextStroke @JvmOverloads constructor(private val text: String?, private val font: Font?, private val stretchToFit: Boolean = true, private val repeat: Boolean = false) : Stroke {
    ...
    override fun createStrokedShape(shape: Shape?): Shape? {
        ...
        val factor = if (stretchToFit) measurePathLength(shape) / glyphVector.logicalBounds.width as Float else 1.0f
        var nextAdvance = 0f
        while (currentChar < length && !it.isDone) {
            ...
            when (type) {
                ...
                PathIterator.SEG_CLOSE -> {
                    ...
                    val dx = thisX - lastX
                    val dy = thisY - lastY
                    val distance = Math.sqrt(dx * dx + dy * dy.toDouble()) as Float
                    if (distance >= next) {
                        val r = 1.0f / distance
                        val angle = Math.atan2(dy.toDouble(), dx.toDouble()) as Float
                        while (currentChar < length && distance >= next) {
                            ...
                            nextAdvance = if (currentChar < length - 1) glyphVector.getGlyphMetrics(currentChar + 1).advance * 0.5f else 0
        ...
        while (!it.isDone) {
            ...
            when (type) {
                ...
                PathIterator.SEG_CLOSE -> {
                    ...
                    val dx = thisX - lastX
                    val dy = thisY - lastY
                    total += Math.sqrt(dx * dx + dy * dy.toDouble()) as Float
                    lastX = thisX
                    lastY = thisY
                }
                PathIterator.SEG_LINETO -> {
