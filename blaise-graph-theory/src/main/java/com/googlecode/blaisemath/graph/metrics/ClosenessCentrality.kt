 ...
 */
class ClosenessCentrality : AbstractGraphNodeMetric<Double?>("Closeness centrality") {
    override fun <N> apply(graph: Graph<N?>?, node: N?): Double? {
        ...
        val sum = lengths.values.stream().mapToDouble(ToDoubleFunction { v: Int? -> v }).sum()
        return cptSize / n * (n - 1.0) / sum
    }

    override fun <N> apply(graph: Graph<N?>?): MutableMap<N?, Double?>? {
        val id = Instrument.start("ClosenessCentrality.allValues", graph.nodes().size.toString() + " nodes", graph.edges().size.toString() + " edges")
