 ...
 */
class ClusteringCoefficientByPath : AbstractGraphMetric<Double?>("Clustering coefficient (by path)", "Computes the clustering coefficient:"
        + " Out of all length-3 paths, how many are enclosed by a triangle?", true) {
    override fun apply(graph: Graph<*>?): Double? {
        val tri: IntArray = ClusteringCoefficient.Companion.triples<Any?>(graph)
        var triangles = tri[0]
        val triples = tri[1]
        if (!graph.isDirected()) {
            triangles /= 3
