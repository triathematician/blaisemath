...
class GraphLayoutManager<N> {
    ...
    private val initialLayout: StaticGraphLayout<CircleLayout.CircleLayoutParameters?>? = CircleLayout.Companion.getInstance()

    /** The initial layout parameters  */
    private val initialLayoutParameters: CircleLayout.CircleLayoutParameters? = CircleLayout.CircleLayoutParameters(50)

    /** The layout scheme for adding nodes  */
    private val addingLayout: StaticGraphLayout<CircleLayout.CircleLayoutParameters?>? = PositionalAddingLayout()

    /** The initial layout parameters  */
    private val addingLayoutParameters: CircleLayout.CircleLayoutParameters? = CircleLayout.CircleLayoutParameters(100)
    ...
    private fun initializeNodeLocations(old: Graph<N?>?, g: Graph<N?>?) {
        synchronized(coordinateManager) {
            val oldNodes: MutableSet<N?>? = Sets.difference(coordinateManager.getActive(), g.nodes())
            ...
            if (coordinateManager.locatesAll(g.nodes())) {
                ...
                newLoc = if (old == null) {
                    ...
                    val curLoc = coordinateManager.getActiveLocationCopy()
                    addingLayout.layout(g, curLoc, addingLayoutParameters)
                }
                // remove objects that are already in coordinate manager
                newLoc.keys.removeAll(coordinateManager.getActive())
                newLoc.keys.removeAll(coordinateManager.getInactive())
                coordinateManager.reactivate(g.nodes())
                coordinateManager.putAll(newLoc)
            }

            // log size mismatches to help with debugging
            val sz = coordinateManager.getActive().size
