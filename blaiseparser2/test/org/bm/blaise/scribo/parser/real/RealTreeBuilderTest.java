 ...
 * @author ae3263
 */
public class RealTreeBuilderTest {

    public RealTreeBuilderTest() {
    }
    ...
    void assertEqualTree(double value, String s2) {
        ...
        double treeValue = 0;
        try {
            tree = RealTreeBuilder.INSTANCE.buildTree(s2);
            try {
                treeValue = (Double) tree.value();
                assertEquals(value, treeValue, 1e-5);
            } catch (SemanticTreeEvaluationException ex) {
                Logger.getLogger(RealTreeBuilderTest.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public void testBuildTreeRandom() {
        ...
        assertEqualTree(0.0, "rand(0,1e-6)");
        assertEqualTree(0.0, "random(0,1e-6)");
