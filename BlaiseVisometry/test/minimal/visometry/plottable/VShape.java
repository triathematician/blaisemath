...
public class VShape<C> extends VPointSet<C> {
    ...
    VGraphicEntry entry2;

    /** Constructs a dynamic shape that passes through the specified value. */
    public VShape(C... values) {
        super(values);
        entry.visible = false;
        addPrimitive(entry2 = new VGraphicEntry(points, new ShapeStyle(Color.BLACK, Color.LIGHT_GRAY)));
    }
    ...
    public ShapeStyle getShapeStyle() { return (ShapeStyle) entry2.renderer; }
    /** Set current style of stroke for this plottable */
    public void setShapeStyle(ShapeStyle newValue) { if (entry2.renderer != newValue) { entry2.renderer = newValue; firePlottableStyleChanged(); } }
