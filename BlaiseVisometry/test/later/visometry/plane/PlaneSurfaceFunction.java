...
import graphics.GraphicPointRadius;
 ...
 */
public class PlaneSurfaceFunction extends AbstractPlottable<Point2D.Double> {
    ...
    public PlaneSurfaceFunction(MultivariateRealFunction func) {
        addPrimitive(entry = new VGraphicEntry(null, new PointRadiusStyle()));
        setFunction(func);
    }
    ...
    protected void recompute() {
        ...
        ((PointRadiusStyle)entry.renderer).setMaxRadius(DEFAULT_PIXEL_SPACING * radiusMultiplier);
    ...
    public PointRadiusStyle getStyle() { return (PointRadiusStyle) entry.renderer; }
    /** Set current style of stroke for this plottable */
    public void setStyle(PointRadiusStyle newValue) { if (entry.renderer != newValue) { entry.renderer = newValue; firePlottableStyleChanged(); } }
