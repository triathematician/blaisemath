...
public class Euclidean3 extends Euclidean2 {
    ...
    /** Left color */
    public final Color leftColor = new Color (0, 255, 255, 225);
    /** Right color */
    public final Color rightColor = new Color (255, 0, 0, 225);
    ...
    public class ViewProjection implements Function<R3,R2> {        
        ...
        DoubleRangeModel eyeSep = new DoubleRangeModel(1.5,0.1,5.0,0.01);
        ...
        /** Returns central camera point. */
        public R3 getCamera() { 
            la = 1;
            lb = 1;
            return center.minus(tDir.times(viewDist.getValue())); 
        }
        /** Returns left eye camera. */
        public R3 getLeftCamera() {
            la = 1;
            lb = 1;
            return center.minus(tDir.times(viewDist.getValue())).plus(nDir.times(-eyeSep.getValue()/2));
        }
        /** Returns left eye camera. */
        public R3 getRightCamera() {
            la = 1;
            lb = 1;
            return center.minus(tDir.times(viewDist.getValue())).plus(nDir.times(+eyeSep.getValue()/2));
        }
        ...
        public R2 getValue(R3 pt) throws FunctionValueException {
            ...
            if (dist < clipDist.getValue()) { return null; }
            return new R2(diff.dotProduct(nDir)*la, diff.dotProduct(bDir)*lb).times(viewDist.getValue()/dist);
        }
        ...
        public Vector<R2> getValue(Vector<R3> pts) throws FunctionValueException {
            ...
            for (R3 pt : pts) {
                ...
                if (dist < clipDist.getValue()) {
                    ...
                    result.add(new R2(diff.dotProduct(nDir)*la, diff.dotProduct(bDir)*lb).times(viewDist.getValue()/dist));
                }
            }   
            return result;
        }
        ...
        public R3 getIValue(R2 pt, R3 t0, R3 n0, R3 b0) {
            return center.minus(t0.times(sceneSize.getValue())).plus( n0.times(pt.x/la) ).plus (b0.times(pt.y/lb) );
        }
