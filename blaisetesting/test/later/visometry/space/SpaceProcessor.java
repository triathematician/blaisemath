 ...
 */
public class SpaceProcessor extends VisometryProcessor<Point3D> {

    @Override
    protected void convert(VGraphicEntry entry, Visometry<Point3D> vis) {
        if (entry.local == null)
            return;

        if (entry.local instanceof Point3D)
        {
            entry.primitive = vis.toWindow((Point3D) entry.local);
        }
        else if (entry.local instanceof Point3D[] && (entry.renderer instanceof PathStyleShape || entry.renderer instanceof ShapeStyle))
        {
            Point3D[] locArr = (Point3D[]) entry.local;
            GeneralPath path = new GeneralPath();
            Point2D.Double winPt = vis.toWindow(locArr[0]);
            path.moveTo((float) winPt.x, (float) winPt.y);
            for (int i = 0; i < locArr.length; i++) {
                winPt = vis.toWindow(locArr[i]);
        ...
        else if (entry.local instanceof GraphicString)
        {
            ...
            if (gs.getAnchor() instanceof Point3D)
                entry.primitive = new GraphicString<Point2D.Double>(vis.toWindow((Point3D) gs.getAnchor()), gs.getString());
        } 
        else if (entry.local instanceof GraphicMesh)
        {
            ...
            for (int i = 0; i < winArr.length; i++)
                winArr[i] = vis.toWindow((Point3D) lMesh.points[i]);
