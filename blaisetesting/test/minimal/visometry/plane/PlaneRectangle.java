...
import primitive.style.temp.HandleStyle;
...
public class PlaneRectangle extends DynamicPlottable<Point2D.Double>
    ...
    DraggableVGraphicEntry entry;
    /** Entry containing the control boxes. */
    DraggableVGraphicEntry controlEntry;
    ...
    public PlaneRectangle(double minX, double minY, double maxX, double maxY) {
        ...
        this.maxX = maxX;
        this.maxY = maxY;
        addPrimitive(entry = new DraggableVGraphicEntry(new Rectangle2D.Double(), new ShapeStyle(Color.BLACK, Color.GRAY), this));
        addPrimitive(controlEntry = new DraggableVGraphicEntry(null, new HandleStyle(), this));
    ...
    public ShapeStyle getStyle() { return (ShapeStyle) entry.renderer; }
    /** Set current style of stroke for this plottable */
    public void setStyle(ShapeStyle newValue) { if (entry.renderer != newValue) { entry.renderer = newValue; firePlottableStyleChanged(); } }
