...
object Images {
    ...
    fun encodeStandardBase64(image: BufferedImage?, format: String?): String? {
        Preconditions.checkArgument(IMAGE_FORMATS.contains(format.toLowerCase()), "Unsupported format: $format")
    ...
    fun decodeDataUriBase64(base64: String?): BufferedImage? {
        Preconditions.checkArgument(base64.startsWith(DATA_URI_PREFIX), "Invalid data URI: $base64")
    ...
    fun renderImage(view: Component?): BufferedImage? {
        val image = BufferedImage(view.getWidth(), view.getHeight(), BufferedImage.TYPE_INT_RGB)
        val canvas = image.createGraphics()
        canvas.clip = Rectangle2D.Double(0, 0, view.getWidth(), view.getHeight())
        view.paint(canvas)
        canvas.dispose()
        return image
    }
