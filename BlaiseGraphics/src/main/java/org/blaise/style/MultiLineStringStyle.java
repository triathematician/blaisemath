...
public class MultiLineStringStyle extends BasicStringStyle {
    ...
    public void draw(Point2D point, String string, Graphics2D canvas, Set<VisibilityHint> visibility) {
        ...
        canvas.setClip(clip);
        //clip = new Rectangle2D.Float(0,0,500,500);
        //canvas.setClip(clip);
...
//        float sz = canvas.getFont().getSize2D();
        ...
        for (String s : lns){
            ...
            //Rectangle2D rectTemp = canvas.getFontMetrics(f).getStringBounds(s,canvas);
            //boundW = Math.max(boundW, (float)rectTemp.getWidth());
            //boundH = boundH + (float)rectTemp.getHeight(); 
            lines.add(s);
        }
        
        
        
        //clip.setFrame(0, 0, boundW, boundH);
