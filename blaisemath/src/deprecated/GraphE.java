 ...
 */
@Deprecated
public class GraphE extends TreeSet<EdgeE> {
    ...
    public GraphE clone() {
        ...
        for (EdgeE e : this) {
            g.addEdge(e.getSource(), e.getSink(), e.getWeight());
        }
        return g;
    }
    ...
    public EdgeE getEdge(int a, int b) {
        for (EdgeE e : this) {
            if (e.equals(a, b)) {
                return e;
            }
    ...
    public boolean addTrivialLoop() {
        return addEdge(-1, -1, 1);
    }

    public void deleteTrivialLoops() {
        removeVertex(-1);
    }

    public boolean addEdge(EdgeE e) {
        return addEdge(e.v0, e.v1, e.weight);
    }
    ...
    public void removeEdge(int ev0, int ev1) {
        remove(getEdge(ev0, ev1));
    }
    ...
    public void removeEdge(int ev0, int ev1, int w) {
        if (multiEdge) {
            ...
            this.remove(new EdgeE(ev0, ev1, w));
        }
    }
    ...
    public int getMaxVertex() {
        ...
        return getVertices().last();
    }

    /** Returns the number of trivial loops. */
    public int getNumTrivialLoops() {
        return getLoopsAt(-1);
    }
    ...
    public String toString() {
        String s = "{";
        if (getNumTrivialLoops() > 0) {
            s += "(" + getNumTrivialLoops() + ")";
        }
        for (EdgeE e : this) {
            if (!e.contains(-1)) {
                s += e.toString(multiEdge, directed);
            }
        }
        return s + "}";
    }
    ...
    public void removeVertex(int v) {
        TreeSet<EdgeE> removals = new TreeSet<EdgeE>();
        for (EdgeE e : this) {
            if (e.contains(v)) {
                removals.add(e);
            }
    ...
    public int getLoopsAt(int v) {
        float i = 0;
        for (EdgeE e : this) {
            i += e.weightedEquals(v, v);
        }
    ...
    public GraphE getComponent(int v) {
        ...
        do {
            ...
            for (Integer i : newVertices) {
                used.addAll(unused.getNeighborhood(i));
            }
    ...
    public ArrayList<GraphE> getAllComponents() {
        ...
        verticesUnused.add(-1);
        verticesUnused.addAll(getVertices());
    ...
    public void addToLabelsAbove(int n, int i) {
        for (EdgeE e : this) {
            if (!e.isTrivial()) {
                e.addToLabelAbove(n, i);
            }
        }
    }
    ...
    public void reverseLabels() {
        Integer[] verts = (Integer[]) getVertices().toArray();
        TreeMap<Integer, Integer> mapping = new TreeMap<Integer, Integer>();
        mapping.put(-1, -1);
        for (int i = 0; i < verts.length; i++) {
            mapping.put(verts[i], verts[verts.length - 1 - i]);
        }
        for (EdgeE e : this) {
            e.relabel(mapping);
        }
    ...
    public int glueTo(GraphE g, Map<Integer, Integer> map) {
        ...
        if (map != null) {
            for (int key : map.keySet()) {
                glueVertices(key + plus, map.get(key));
            }
        }
        return plus;
    }
    ...
    public GraphE surgery(GraphE x) {
        return null;
    }

    /** Performs closing operation */
    public GraphE closeOff() {
        return null;
    }
    ...
    public float[][] getWeightedMatrix() {
        ...
        for (EdgeE e : this) {
            matrix[e.getSource()][e.getSink()] = e.getWeight();
        }
        return matrix;
    }
    ...
    public float[][] getOrderedMatrix() {
        ...
        float[][] matrix = new float[n][n];
        for (EdgeE e : this) {
            matrix[e.getSource()][e.getSink()] = 1;
        }
        return matrix;
    }
    ...
    public float[][] getUnOrderedMatrix() {
        ...
        for (EdgeE e : this) {
            matrix[e.getSource()][e.getSink()] = 1;
            matrix[e.getSink()][e.getSource()] = 1;
        }
        return matrix;
    }
}
