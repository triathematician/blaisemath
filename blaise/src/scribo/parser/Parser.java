...
public class Parser {
    ...
    public static boolean mismatchedParentheses(String s) {
        ...
        if(verbose){System.out.println("mismatchedParentheses input: "+s);}
        Matcher mp=innerParentheticalPattern.matcher(s);
        while(mp.find()){
            s=mp.replaceFirst("");
            mp=innerParentheticalPattern.matcher(s);
            if(verbose){System.out.println("  checkpoint: "+s);}
        }
    ...
    public static Vector<FunctionTreeNode> operatorSplit(String op,List<String> operators,List<FunctionTreeNode> expressions)
        ...
        if (expressions.size()!=n+1){return null;}
        int firstOp=operators.indexOf(op);
        if (firstOp==-1){return null;}
        Vector<FunctionTreeNode> result=new Vector<FunctionTreeNode>();
        if (firstOp==0){result.add(expressions.get(0));}
        else{result.add(resolveOperators(operators.subList(0,firstOp),expressions.subList(0,firstOp+1)));}
        for(int i=firstOp+1;i<operators.size();i++){
            if(!operators.get(i).equals(op)){continue;}
    ...
    public static FunctionTreeNode resolveOperators(List<String> operators, List<FunctionTreeNode> expressions) throws FunctionSyntaxException  {
        if(verbose){
            ...
            try{
                String temp="";for(int i=0;i<expressions.size();i++){temp+=expressions.get(i).toString()+(i!=expressions.size()?", ":"");}
        ...
        opNodes=operatorSplit("-",operators,expressions);
        if(opNodes!=null){return Operation.subtractNode(opNodes);}
        opNodes=operatorSplit("*",operators,expressions);
        if(opNodes!=null){return new Operation.Multiply(opNodes);}
        opNodes=operatorSplit("/",operators,expressions);
    ...
    public static FunctionTreeNode parseExpression(String s) throws FunctionSyntaxException {   
        ...
        if(verbose){System.out.println("  space handled 1="+s);}
        mp=insidePSpacePattern.matcher(s);
        if(mp.find()){ 
            mp.reset();
            StringBuffer sb=new StringBuffer();
            while(mp.find()){mp.appendReplacement(sb,mp.group(1));}
            mp.appendTail(sb);
            s=sb.toString();
        }
        
        if(verbose){System.out.println("  space handled 1.5="+s);}
        for(int i=0;i<implicitStarPattern.length;i++){
            mp=implicitStarPattern[i].matcher(s);
            if(mp.find()){ 
                mp.reset();
                StringBuffer sb=new StringBuffer();
                while(mp.find()){mp.appendReplacement(sb,mp.group(1)+"*"+mp.group(2));}
                mp.appendTail(sb);
                s=sb.toString();
            }
        }
        if(verbose){System.out.println("  space handled 2="+s);}
        s.replaceAll(XSPACE,"");
        if(verbose){System.out.println("  space handled 3="+s);}
        
        // STEP 1B. HANDLE STARTING WITH -
        if(s.startsWith("-")){s="0"+s;}
        if(verbose){System.out.println("  space handled 4="+s);}
        ...
        if(verbose){System.out.println("  operator find 5="+positions.toString());}
