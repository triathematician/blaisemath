...
public class Point2D extends DynamicPlottable<Euclidean2> implements ChangeListener {
    ...
    public Point2D() {
        this(new PointRangeModel());
    }

    /** Constructor places point at a given location. */
    public Point2D(R2 value) {
        this(new PointRangeModel(value));
    }

    public Point2D(double x, double y) {
        this(new PointRangeModel(x, y));
    }

    public Point2D(Color c) {
        this();
        setColor(c);
    }
    ...
    public Point2D(PointRangeModel prm, Color c) {
        this(prm);
        setColor(c);
    }

    public Point2D(R2 point, Color c, boolean editable) {
        this(point.x, point.y, c, editable);
    }
    ...
    public Point2D(double x, double y, Color color, boolean editable) {
        ...
        if (editable) {
            prm = new PointRangeModel(x, y);
        } else {
            prm = new PointRangeModel(new R2(x, y), x, y, x, y);
        }
        if (editable) {
    ...
    public PointRangeModel getModel() {
        return prm;
    }
    ...
    @Override
    public boolean clicked(MouseVisometryEvent<Euclidean2> e) {
        return withinClickRange(e, prm.getPoint());
    }

    /** Handles dragging. */
    @Override
    public void mouseDragged(MouseVisometryEvent<Euclidean2> e) {
        if (adjusting) {
            setPoint((R2) e.getCoordinate());
        }
    }

    /** Handles a click by cycling the visual style. */
    @Override
    public void mouseClicked(MouseVisometryEvent<Euclidean2> e) {
        if (clicked(e)) {
            style.cycle();
            adjusting = false;
        }
