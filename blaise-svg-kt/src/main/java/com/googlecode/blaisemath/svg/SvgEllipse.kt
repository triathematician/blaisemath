...
class SvgEllipse @JvmOverloads constructor(private var cx: Double = 0.0, private var cy: Double = 0.0, private var rx: Double = 0.0, private var ry: Double = 0.0) : SvgElement("ellipse") {
    ...
    //</editor-fold>
    private class EllipseConverter : Converter<SvgEllipse?, Ellipse2D?>() {
        protected fun doBackward(r: Ellipse2D?): SvgEllipse? {
            return SvgEllipse(r.getCenterX(), r.getCenterY(), r.getWidth() / 2, r.getHeight() / 2)
        }

        protected fun doForward(r: SvgEllipse?): Ellipse2D? {
            return Ellipse2D.Double(r.cx - r.rx, r.cy - r.ry, 2 * r.rx, 2 * r.ry)
        }
    }

    companion object {
        private val CONVERTER_INST: EllipseConverter? = EllipseConverter()
        fun shapeConverter(): Converter<SvgEllipse?, Ellipse2D?>? {
