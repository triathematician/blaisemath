...
class SvgCircle @JvmOverloads constructor(private var cx: Double = 0.0, private var cy: Double = 0.0, private var r: Double = 0.0) : SvgElement("circle") {
    ...
    //</editor-fold>
    private class CircleConverter : Converter<SvgCircle?, Ellipse2D?>() {
        protected fun doBackward(r: Ellipse2D?): SvgCircle? {
            checkArgument(r.getWidth() == r.getHeight(), "Ellipse must have width=height")
            return SvgCircle(r.getCenterX(), r.getCenterY(), r.getWidth() / 2)
        }

        protected fun doForward(r: SvgCircle?): Ellipse2D? {
            return Ellipse2D.Double(r.cx - r.r, r.cy - r.r, 2 * r.r, 2 * r.r)
        }
    }

    companion object {
        private val CONVERTER_INST: CircleConverter? = CircleConverter()
        fun shapeConverter(): Converter<SvgCircle?, Ellipse2D?>? {
