...
open class SvgGroup : SvgElement("g") {
    private var obj: MutableList<SvgElement?>? = Lists.newArrayList()
    ...
    //
    @XmlElementRef
    fun getElements(): MutableList<SvgElement?>? {
        return obj
    }

    fun setElements(obj: MutableList<SvgElement?>?) {
        this.obj = obj
    }

    fun addElement(obj: SvgElement?) {
        this.obj.add(checkNotNull(obj))
    }

    //</editor-fold>
    fun getObjectById(id: String?): SvgElement? {
        for (ms in obj) {
    ...
    companion object {
         ...
         * @return group
         */
        fun create(vararg elements: SvgElement?): SvgGroup? {
            val res = SvgGroup()
            for (el in elements) {
                res.addElement(el)
            }
