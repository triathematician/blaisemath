...
class SvgPolygon : SvgElement {
    private var ptStr: String? = ""

    constructor() : super("polygon") {}
    constructor(pts: String?) : super("polygon") {
        ptStr = SvgPolyline.Companion.checkPointString(pts)
    }
    ...
    fun setPointStr(pathStr: String?) {
        ptStr = SvgPolyline.Companion.checkPointString(pathStr)
    }

    private class PolygonConverter : Converter<SvgPolygon?, GeneralPath?>() {
        protected fun doForward(a: SvgPolygon?): GeneralPath? {
            val gp: GeneralPath = SvgPolyline.Companion.toPath(a.ptStr)
            gp.closePath()
            return gp
        }

        protected fun doBackward(b: GeneralPath?): SvgPolygon? {
            val s: String = SvgPolyline.Companion.toPathString(b)
            return SvgPolygon(s)
        }
    }

    companion object {
        private val CONVERTER_INST: PolygonConverter? = PolygonConverter()

        //</editor-fold>
        fun shapeConverter(): Converter<SvgPolygon?, GeneralPath?>? {
