...
class SvgRectangle @JvmOverloads constructor(private var x: Double = 0.0, private var y: Double = 0.0, private var width: Double = 0.0, private var height: Double = 0.0, private var rx: Double = 0.0, private var ry: Double = 0.0) : SvgElement("rect") {
    ...
    private class RectangleConverter : Converter<SvgRectangle?, RectangularShape?>() {
        fun doBackward(r: RectangularShape?): SvgRectangle? {
            checkArgument(r is RoundRectangle2D || r is Rectangle2D,
                    "Invalid shape: $r")
            return if (r is RoundRectangle2D) {
                val rr = r as RoundRectangle2D?
                SvgRectangle(rr.getX(), rr.getY(), rr.getWidth(), rr.getHeight(), rr.getArcWidth(), rr.getArcHeight())
            } else if (r is Rectangle2D) {
                SvgRectangle(r.getX(), r.getY(), r.getWidth(), r.getHeight())
            } else {
                throw IllegalStateException()
            }
        }

        fun doForward(r: SvgRectangle?): RectangularShape? {
            return if (r.rx == 0.0 && r.ry == 0.0) {
                Rectangle2D.Double(r.x, r.y, r.width, r.height)
            } else {
                RoundRectangle2D.Double(r.x, r.y, r.width, r.height, r.rx, r.ry)
            }
        }
    }

    companion object {
        private val CONVERTER_INST: RectangleConverter? = RectangleConverter()

        /**
         * Get converter that translates an [SvgRectangle] to/from a [RectangularShape].
         * @return converter instance
         */
        fun shapeConverter(): Converter<SvgRectangle?, RectangularShape?>? {
