...
class SvgPolyline : SvgElement {
    private var ptStr: String? = null

    constructor() : super("polyline") {}
    constructor(pts: String?) : super("polyline") {
        ptStr = checkPointString(pts)
    }
    ...
    fun setPointStr(ptStr: String?) {
        this.ptStr = checkPointString(ptStr)
    }

    private class PolylineConverter : Converter<SvgPolyline?, GeneralPath?>() {
        protected fun doForward(a: SvgPolyline?): GeneralPath? {
            return toPath(a.ptStr)
        }

        protected fun doBackward(b: GeneralPath?): SvgPolyline? {
            return SvgPolyline(toPathString(b))
        }
    }

    companion object {
        private val CONVERTER_INST: PolylineConverter? = PolylineConverter()

        //</editor-fold>
        fun shapeConverter(): Converter<SvgPolyline?, GeneralPath?>? {
            return CONVERTER_INST
        }
        //<editor-fold defaultstate="collapsed" desc="point string utilities">
        /** Ensures validity of a string of points  */
        fun checkPointString(s: String?): String? {
            checkNotNull(s)
            val ps = s.replace("\\s*,\\s*".toRegex(), ",")
            val pp: Array<String?> = ps.split("\\s+".toRegex()).toTypedArray()
            for (p in pp) {
                val p2: Array<String?> = p.split(",".toRegex()).toTypedArray()
                checkArgument(p2.size == 2, "Invalid point string: $s")
                try {
                    java.lang.Double.valueOf(p2[0])
                    java.lang.Double.valueOf(p2[1])
                } catch (x: NumberFormatException) {
                    throw IllegalArgumentException("Invalid point string: $s", x)
                }
            }
            return s
        }
        ...
        fun toPath(pointStr: String?): GeneralPath? {
            checkPointString(pointStr)
            val ps = pointStr.replace("\\s*,\\s*".toRegex(), ",")
            val path = GeneralPath()
            for (p in ps.split("\\s+".toRegex()).toTypedArray()) {
                val sp2: Array<String?> = p.split(",".toRegex()).toTypedArray()
                if (path.currentPoint == null) {
                    path.moveTo(java.lang.Double.valueOf(sp2[0]), java.lang.Double.valueOf(sp2[1]))
                } else {
                    path.lineTo(java.lang.Double.valueOf(sp2[0]), java.lang.Double.valueOf(sp2[1]))
                }
            }
            return path
        }
        ...
        fun toPathString(gp: GeneralPath?): String? {
            ...
            while (!pi.isDone) {
                ...
                if (curSegmentType == PathIterator.SEG_LINETO || curSegmentType == PathIterator.SEG_MOVETO) {
                    val s = " " + SvgPath.Companion.numStr(",", 6, cur[0], cur[1])
                    pathString.append(s)
                    if (s0 == null) {
                        s0 = s
                    }
                    ...
                    throw IllegalArgumentException("Path cannot be converted to polyline.")
                }
                pi.next()
            }
