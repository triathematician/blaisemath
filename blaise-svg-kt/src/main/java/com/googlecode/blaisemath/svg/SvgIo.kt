 ...
 * @author elisha
 */
internal object SvgIo {
    private var CONTEXT: JAXBContext? = null
    @Throws(JAXBException::class)
    fun context(): JAXBContext? {
        if (CONTEXT == null) {
            CONTEXT = JAXBContext.newInstance(SvgRoot::class.java)
        }
        return CONTEXT
    }
    ...
    fun flexibleNamespaceParse(input: InputSource?): SvgRoot? {
        return try {
            val filter: XMLFilter = SvgNamespaceFilter()
            val factory = SAXParserFactory.newInstance()
            factory.isNamespaceAware = true
            filter.parent = factory.newSAXParser().xmlReader
            val umHandler: UnmarshallerHandler = unmarshaller().getUnmarshallerHandler()
            filter.contentHandler = umHandler
            filter.parse(input)
            umHandler.getResult() as SvgRoot
        } catch (x: ParserConfigurationException) {
            throw IOException("Invalid svg or other error: $input", x)
        } catch (x: SAXException) {
            throw IOException("Invalid svg or other error: $input", x)
        } catch (x: JAXBException) {
            throw IOException("Invalid svg or other error: $input", x)
        }
    }
    ...
    @Throws(IOException::class)
    fun read(input: String?): SvgRoot? {
        return read(StringReader(input))
    }

    @Throws(IOException::class)
    fun read(input: InputStream?): SvgRoot? {
        return flexibleNamespaceParse(InputSource(input))
    }

    @Throws(IOException::class)
    fun read(reader: Reader?): SvgRoot? {
        return flexibleNamespaceParse(InputSource(reader))
    }

    @Throws(IOException::class)
    fun writeToString(root: SvgRoot?): String? {
        val sw = StringWriter()
        write(root, sw)
        return sw.toString()
    }
    ...
    @Throws(IOException::class)
    fun write(root: SvgRoot?, output: OutputStream?) {
        try {
            marshaller().marshal(root, output)
        } catch (ex: JAXBException) {
            throw IOException("Could not save SvgRoot to output", ex)
        }
    }

    @Throws(IOException::class)
    fun write(root: SvgRoot?, writer: Writer?) {
        try {
