 ...
 * @author petereb1
 */
class SvgGraphicComponent : JGraphicComponent() {
    protected val graphic: SvgGraphic? = SvgGraphic()
    ...
    fun getSvgText(): String? {
        return try {
            SvgRoot.Companion.saveToString(graphic.getElement())
        } catch (x: IOException) {
    ...
    fun setSvgText(svg: String?) {
        try {
            setElement(SvgRoot.Companion.load(svg))
        } catch (x: IOException) {
    ...
    companion object {
        private val LOG = Logger.getLogger(SvgGraphicComponent::class.java.name)
         ...
         * @return component
         */
        fun create(svg: SvgElement?): SvgGraphicComponent? {
            val res = SvgGraphicComponent()
            res.setElement(svg)
            return res
        }
         ...
         * @return component
         */
        fun create(svg: String?): SvgGraphicComponent? {
            val res = SvgGraphicComponent()
            res.setSvgText(svg)
            val bounds = res.graphic.getGraphicBounds()
            if (bounds != null) {
                res.preferredSize = Dimension(bounds.width as Int + 1, bounds.height as Int + 1)
            }
            return res
        }
    }
