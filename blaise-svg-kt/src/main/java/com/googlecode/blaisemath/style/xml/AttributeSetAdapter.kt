...
/**
 * Allows representing [AttributeSet]s in XML as strings.
 * @author elisha
 */
class AttributeSetAdapter : XmlAdapter<String?, AttributeSet?>() {
    fun unmarshal(v: String?): AttributeSet? {
        val res = AttributeSetCoder().decode(v)
        if (res != null) {
    ...
    fun marshal(v: AttributeSet?): String? {
        return AttributeSetCoder().encode(v)
    }

    companion object {
        private val LOG = Logger.getLogger(AttributeSetAdapter::class.java.name)
        private val COLOR_KEYS: Array<String?>? = arrayOf(Styles.FILL, Styles.STROKE)
        private val NUMBER_KEYS: Array<String?>? = arrayOf(Styles.STROKE_WIDTH)
         ...
         */
        fun updateColorFields(style: AttributeSet?) {
            for (s in COLOR_KEYS) {
                if (style.get(s) is String) {
                    val color = decodeColorString(style.get(s) as String?)
                    if (color != null) {
                        style.put(s, color)
                    }
                }
         ...
         * @param style attribute set
         */
        fun updateNumberFields(style: AttributeSet?) {
            for (s in NUMBER_KEYS) {
                val value = style.get(s)
                if (value is String && (value as String?).endsWith("px")) {
                    var sval = value as String?
                    sval = sval.substring(0, sval.length - 2)
                    try {
                        val f = java.lang.Float.valueOf(sval)
                        style.put(s, f)
        ...
        private fun decodeColorString(s: String?): Color? {
            val name = s.toLowerCase()
            try {
                return Color::class.java.getField(name)[null] as Color
