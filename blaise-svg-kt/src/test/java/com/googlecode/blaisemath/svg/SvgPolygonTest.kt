 ...
 * @author elisha
 */
class SvgPolygonTest : TestCase() {
    @Test
    fun testConvertToSvg() {
        println("convertToSvg")
        val conv: Converter<SvgPolygon?, GeneralPath?> = SvgPolygon.Companion.shapeConverter()
        val gp1 = GeneralPath()
        gp1.moveTo(0f, 0f)
        gp1.lineTo(1f, 1f)
        gp1.closePath()
    ...
    fun testConvertFromSvg() {
        println("convertToSvg")
        val conv: Converter<SvgPolygon?, GeneralPath?> = SvgPolygon.Companion.shapeConverter()
        val poly1 = SvgPolygon("0,0 1,0 0,1")
        ...
        try {
            SvgPolygon("not points")
            fail("Shouldn't be able to create")
        } catch (x: IllegalArgumentException) {
            // expected
        }
        try {
            SvgPolygon("1,2,3 4,5 6,7")
            fail("Shouldn't be able to create")
        } catch (x: IllegalArgumentException) {
            // expected
        }
        try {
            SvgPolygon("1,a 4,5 6,7")
            fail("Shouldn't be able to create")
        } catch (x: IllegalArgumentException) {
            // expected
        }
