 ...
 * @author elisha
 */
class SvgPolylineTest : TestCase() {
    @Test
    fun testConvertToSvg() {
        println("convertToSvg")
        val conv: Converter<SvgPolyline?, GeneralPath?> = SvgPolyline.Companion.shapeConverter()
        val gp1 = GeneralPath()
        gp1.moveTo(0f, 0f)
        gp1.lineTo(1f, 1f)
        gp1.closePath()
    ...
    fun testConvertFromSvg() {
        println("convertToSvg")
        val conv: Converter<SvgPolyline?, GeneralPath?> = SvgPolyline.Companion.shapeConverter()
        val poly1 = SvgPolyline("0,0 1,0 0,1")
        val gp1: GeneralPath = conv.convert(poly1)
        val pi = gp1.getPathIterator(null)
        val crd = DoubleArray(6)
        ...
        try {
            SvgPolyline("not points")
            fail("Shouldn't be able to create")
        } catch (x: IllegalArgumentException) {
            // expected
        }
        try {
            SvgPolyline("1,2,3 4,5 6,7")
            fail("Shouldn't be able to create")
        } catch (x: IllegalArgumentException) {
            // expected
        }
        try {
            SvgPolyline("1,a 4,5 6,7")
            fail("Shouldn't be able to create")
        } catch (x: IllegalArgumentException) {
            // expected
        }
