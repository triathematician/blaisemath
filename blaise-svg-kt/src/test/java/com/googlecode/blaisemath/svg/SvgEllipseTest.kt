 ...
 * @author elisha
 */
class SvgEllipseTest : TestCase() {
    @Test
    fun testConvertToSvg() {
        println("convertToSvg")
        val conv: Converter<SvgEllipse?, Ellipse2D?> = SvgEllipse.Companion.shapeConverter()
        val e1: Ellipse2D = Ellipse2D.Double(1.0, 2.0, 2.0, 3.0)
        val circ: SvgEllipse = conv.reverse().convert(e1)!!
        assertEquals(2.0, circ.cx, 1e-6)
        assertEquals(3.5, circ.cy, 1e-6)
        assertEquals(1.0, circ.rx, 1e-6)
        assertEquals(1.5, circ.ry, 1e-6)
    ...
    fun testConvertFromSvg() {
        println("convertToSvg")
        val conv: Converter<SvgEllipse?, Ellipse2D?> = SvgEllipse.Companion.shapeConverter()
        val sEll = SvgEllipse(1, 2, 3, 2)
        val ell: Ellipse2D = conv.convert(sEll)!!
        assertEquals(-2.0, ell.minX, 1e-6)
        assertEquals(0.0, ell.minY, 1e-6)
        assertEquals(6.0, ell.width, 1e-6)
        assertEquals(4.0, ell.height, 1e-6)
