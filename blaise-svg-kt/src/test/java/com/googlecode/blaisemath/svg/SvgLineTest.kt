 ...
 * @author elisha
 */
class SvgLineTest : TestCase() {
    @Test
    fun testConvertToSvg() {
        println("convertToSvg")
        val conv: Converter<SvgLine?, Line2D?> = SvgLine.Companion.shapeConverter()
        val l1: Line2D = Line2D.Double(1.0, 2.0, 4.0, 3.0)
        val line: SvgLine = conv.reverse().convert(l1)!!
        assertEquals(1.0, line.x1, 1e-6)
        assertEquals(2.0, line.y1, 1e-6)
        assertEquals(4.0, line.x2, 1e-6)
        assertEquals(3.0, line.y2, 1e-6)
    ...
    fun testConvertFromSvg() {
        println("convertToSvg")
        val conv: Converter<SvgLine?, Line2D?> = SvgLine.Companion.shapeConverter()
        val sLine = SvgLine(1, 2, 3, 4)
        val line: Line2D = conv.convert(sLine)!!
        assertEquals(1.0, line.x1, 1e-6)
        assertEquals(2.0, line.y1, 1e-6)
        assertEquals(3.0, line.x2, 1e-6)
        assertEquals(4.0, line.y2, 1e-6)
