...
class SvgPathTest {
    ...
    @Test
    fun testSvgRel() {
        TestCase.assertEquals("M 10 10 L 11 10 L 11 11", recyclePath("M10 10l1 0 0 1"))
    }

    @Test
    fun testSvgCloseMove() {
        TestCase.assertEquals("M 0 0 L 1 0 ZM 1 1 L 2 2", recyclePath("M 0 0 L 1 0 Z m 1 1 l 1 1"))
    }

    @Test
    fun testSvgRel2() {
        TestCase.assertEquals("M 600 350 L 650 325 L 650 325.000031 C 643.096436 311.192902 648.692871 294.403564 662.5 287.500031 C 676.307129 280.596466 693.096436 286.192902 700 300.000031",
                recyclePath("M600,350 l 50,-25a25,25 -30 0,1 50,-25"))
    }

    companion object {
        private fun toPath(p: String?): Path2D? {
            return SvgPath.Companion.shapeConverter().convert(SvgPath(p))
        }

        private fun recyclePath(path: String?): String? {
            return SvgPath.Companion.shapeConverter().reverse().convert(toPath(path)).getPathStr()
        }
    }
}