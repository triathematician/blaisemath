 ...
 * @author elisha
 */
class SvgCircleTest : TestCase() {
    @Test
    fun testConvertToSvg() {
        println("convertToSvg")
        val conv: Converter<SvgCircle?, Ellipse2D?> = SvgCircle.Companion.shapeConverter()
        val e1: Ellipse2D = Ellipse2D.Double(1.0, 2.0, 2.0, 2.0)
        val circ: SvgCircle = conv.reverse().convert(e1)!!
        assertEquals(2.0, circ.cx, 1e-6)
        assertEquals(3.0, circ.cy, 1e-6)
        assertEquals(1.0, circ.r, 1e-6)
        val e2: Ellipse2D = Ellipse2D.Double(1.0, 2.0, 1.0, 2.0)
        try {
    ...
    fun testConvertFromSvg() {
        println("convertToSvg")
        val conv: Converter<SvgCircle?, Ellipse2D?> = SvgCircle.Companion.shapeConverter()
        val circ = SvgCircle(1, 2, 3)
        val ell: Ellipse2D = conv.convert(circ)!!
        assertEquals(-2.0, ell.minX, 1e-6)
        assertEquals(-1.0, ell.minY, 1e-6)
        assertEquals(6.0, ell.width, 1e-6)
        assertEquals(6.0, ell.height, 1e-6)
