 ...
 */
class GraphDiameter : AbstractGraphMetric<Int?>("Graph diameter", "Diameter of the graph (longest path between two nodes).", true) {
    override fun apply(graph: Graph?): Int? {
        return applyTyped<Any?>(graph)
    }

    companion object {
        private fun <N> applyTyped(graph: Graph<N?>?): Int? {
