 ...
 */
class EdgeDensity : AbstractGraphMetric<Double?>("Edge density", "Number of edges in the graph divided by the total number possible.", true) {
    override fun apply(graph: Graph?): Double? {
        val n: Int = graph.nodes().size
        return if (graph.isDirected()) graph.edges().size / (n * (n - 1)) else graph.edges().size / (n * (n - 1) / 2.0)
    }
}