 ...
 */
class CycleGraphGenerator : AbstractGraphGenerator("Cycle Graph") {
    override fun apply(p: DefaultGeneratorParameters?): Graph<Int?>? {
        val nodeCount = p.getNodeCount()
        return GraphGenerators.createGraphWithEdges(p,
                object : AbstractList<Array<Int?>?>() {
                    override fun get(index: Int): Array<Int?>? {
                        return arrayOf(index, (index + 1) % nodeCount)
                    }
