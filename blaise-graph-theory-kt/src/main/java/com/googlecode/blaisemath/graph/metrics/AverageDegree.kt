 ...
 */
class AverageDegree : AbstractGraphMetric<Double?>("Average degree", "Average degree of nodes in the graph. Uses average indegree/outdegree for a directed graph.", true) {
    override fun apply(graph: Graph?): Double? {
        return if (graph.isDirected()) graph.edges().size / graph.nodes().size as Double else 2.0 * graph.edges().size / graph.nodes().size as Double
    }
}