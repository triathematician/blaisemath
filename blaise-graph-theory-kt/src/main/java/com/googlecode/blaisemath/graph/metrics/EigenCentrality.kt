...
import com.googlecode.blaisemath.util.Instrument.middle
...
class EigenCentrality : AbstractGraphNodeMetric<Double?>("Eigenvalue centrality (estimated)") {
    ...
    override fun <N> apply(graph: Graph<N?>?): MutableMap<N?, Double?>? {
        val id = start("EigenCentrality.allValues", graph.nodes().size.toString() + " nodes", graph.edges().size.toString() + " edges")
        ...
        middle(id, "EigenCentrality.allValues", "eigenvalues=" + Arrays.toString(div))
