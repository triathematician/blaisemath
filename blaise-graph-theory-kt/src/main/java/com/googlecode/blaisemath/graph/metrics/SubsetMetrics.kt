...
* #L%
*/ /**
 * Utility class for working with [GraphSubsetMetric] instances.
 *
 * @author Elisha Peterson
 */
object SubsetMetrics {
    /**
    ...
    fun <N : Number?> additiveSubsetMetric(baseMetric: GraphNodeMetric<N?>?): GraphSubsetMetric<N?>? {
        return AdditiveSubsetMetric<N?>(baseMetric)
    }
    ...
    fun <N> contractiveSubsetMetric(baseMetric: GraphNodeMetric<N?>?): GraphSubsetMetric<N?>? {
        return ContractiveSubsetMetric<N?>(baseMetric)
    }

    //region INNER CLASSES
    @Beta
    private class AdditiveSubsetMetric<T : Number?>
    /**
     * Constructs with provided base metric.
     * @param baseMetric the metric to use for computations on individual nodes.
     */ private constructor(val baseMetric: GraphNodeMetric<T?>?) : GraphSubsetMetric<T?> {
        override fun <N> getValue(graph: Graph<N?>?, nodes: MutableSet<N?>?): T? {
            var result = 0.0
            var `val`: Number? = null
            for (n in nodes) {
            ...
            if (`val` is Int) {
                return result.toInt() as Int as T
            } else if (`val` is Double) {
                return result as T
            } else if (`val` is Float) {
                return result.toFloat() as Float as T
            }
            return null
        }
    }
    ...
    @Beta
    private class ContractiveSubsetMetric<T>
    /**
     * Constructs with provided base metric.
     * @param baseMetric the metric to use for computations of contracted node
     */ private constructor(val baseMetric: GraphNodeMetric<T?>?) : GraphSubsetMetric<T?> {
        override fun <N> getValue(graph: Graph<N?>?, nodes: MutableSet<N?>?): T? {
            val starNode = Iterables.getFirst(nodes, null)
            val contracted = GraphUtils.contractedGraph(graph, nodes, starNode)
