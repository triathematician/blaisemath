...
class NodeInGraph<E>(node: E?, graph: Graph<E?>?) {
    private val node: E?
    ...
    fun getGraph(): Graph<E?>? {
        return graph
    }

    companion object {
        fun <E> create(node: E?): NodeInGraph<E?>? {
            return NodeInGraph(node, null)
        }
    }
    ...
    init {
        ...
        this.graph = graph
        Preconditions.checkArgument(graph == null || graph.nodes().contains(node))
    }
}