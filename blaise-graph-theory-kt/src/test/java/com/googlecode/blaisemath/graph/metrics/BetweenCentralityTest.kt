...
* #L%
*/   class BetweenCentralityTest {
    @Test
    fun testApply() {
        Assert.assertEquals(9.0, INST1.apply(TEST2, 4), 1e-10)
    }

    @Test
    fun testApply_All() {
        val values = INST1.apply(TEST2)
        Assert.assertEquals(1.0, values[1], 1e-6)
        Assert.assertEquals(1.5, values[2], 1e-6)
        Assert.assertEquals(6.5, values[3], 1e-6)
        Assert.assertEquals(9.0, values[4], 1e-6)
        Assert.assertEquals(5.0, values[5], 1e-6)
        Assert.assertEquals(0.0, values[6], 1e-6)
        Assert.assertEquals(0.0, values[7], 1e-6)
    }

    companion object {
        private var TEST2: Graph<Int?>? = null
        private var INST1: BetweenCentrality? = null
        @BeforeClass
        fun setUpClass() {
