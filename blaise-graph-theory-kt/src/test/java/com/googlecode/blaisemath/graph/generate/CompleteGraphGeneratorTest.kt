...
* #L%
*/   class CompleteGraphGeneratorTest {
    @Test
    fun testGetEmptyGraphInstance() {
        Assert.assertEquals("NODES: [0, 1, 2, 3]  EDGES: 0: [1, 2, 3] 1: [0, 2, 3] 2: [0, 1, 3] 3: [0, 1, 2]",
                GraphUtils.printGraph(CompleteGraphGenerator().apply(DefaultGeneratorParameters(false, 4))))
        Assert.assertEquals("NODES: [0, 1, 2, 3]  EDGES: 0: [1, 2, 3] 1: [0, 2, 3] 2: [0, 1, 3] 3: [0, 1, 2]",
                GraphUtils.printGraph(CompleteGraphGenerator().apply(DefaultGeneratorParameters(true, 4))))
        val result: Graph? = CompleteGraphGenerator().apply(DefaultGeneratorParameters(false, 6))
        val result2: Graph? = CompleteGraphGenerator().apply(DefaultGeneratorParameters(true, 6))
        Assert.assertEquals(6, result.nodes().size.toLong())
        Assert.assertEquals(6, result2.nodes().size.toLong())
        Assert.assertEquals(15, result.edges().size.toLong())
        Assert.assertEquals(30, result2.edges().size.toLong())
        for (i in 0..5) {
            Assert.assertTrue(result.nodes().contains(i))
            Assert.assertTrue(result2.nodes().contains(i))
        }
    }
}