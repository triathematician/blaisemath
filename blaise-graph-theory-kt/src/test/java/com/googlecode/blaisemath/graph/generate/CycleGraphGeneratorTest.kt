...
* #L%
*/   class CycleGraphGeneratorTest {
    @Test
    fun testGetEmptyGraphInstance() {
        Assert.assertEquals("NODES: [0, 1, 2, 3]  EDGES: 0: [1, 3] 1: [0, 2] 2: [1, 3] 3: [0, 2]",
                GraphUtils.printGraph(CycleGraphGenerator().apply(DefaultGeneratorParameters(false, 4))))
        Assert.assertEquals("NODES: [0, 1, 2, 3]  EDGES: 0: [1] 1: [2] 2: [3] 3: [0]",
                GraphUtils.printGraph(CycleGraphGenerator().apply(DefaultGeneratorParameters(true, 4))))
    }
}