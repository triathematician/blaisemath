...
*/   class EdgeLikelihoodGeneratorTest {
    ...
    fun testEdgeLikelihoodGenerator() {
        var result1 = EdgeLikelihoodGenerator().apply(EdgeLikelihoodParameters(false, 10, 0f))
        Assert.assertEquals(10, result1.nodes().size.toLong())
        Assert.assertEquals(0, result1.edges().size.toLong())
        result1 = EdgeLikelihoodGenerator().apply(EdgeLikelihoodParameters(true, 10, 1f))
        Assert.assertEquals(10, result1.nodes().size.toLong())
        Assert.assertEquals(100, result1.edges().size.toLong())
        result1 = EdgeLikelihoodGenerator().apply(EdgeLikelihoodParameters(false, 10, 1f))
        Assert.assertEquals(10, result1.nodes().size.toLong())
        Assert.assertEquals(45, result1.edges().size.toLong())
        result1 = EdgeLikelihoodGenerator().apply(EdgeLikelihoodParameters(false, 10, .25f))
        println("  UNDIRECTED (.25 probability): " + result1.edges().size + " edges, " + GraphUtils.printGraph(result1))
        result1 = EdgeLikelihoodGenerator().apply(EdgeLikelihoodParameters(true, 10, .25f))
