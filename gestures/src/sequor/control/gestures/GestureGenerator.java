...
public abstract class GestureGenerator {
    ...
    public Shape getArcOutput(Vector<Interval<String>> arcInput,Vector<R2> observed){        
        ...
        result.moveTo((float)observed.firstElement().x,(float)observed.firstElement().y);
        R2 start;
        R2 stop=null;
        R2 mid1,mid2;
        for(Interval<String> iv:arcOutput){
            ...
            if(iv.value.equals("Line")){
                result.lineTo((float)start.x,(float)start.y);
                result.lineTo((float)stop.x,(float)stop.y);
            }else if(iv.value.equals("ArcLeft") || iv.value.equals("ArcRight")){
                result.lineTo((float)start.x,(float)start.y);
                result.curveTo((float)mid1.x,(float)mid1.y,(float)mid2.x,(float)mid2.y,(float)stop.x,(float)stop.y);
            }
        }    
