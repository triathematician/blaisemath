 ...
 */
open class TwoPointGraphic(start: Point2D?, end: Point2D?) : GraphicComposite<Graphics2D?>() {
    /** Point at start of arrow  */
    protected val start: PrimitiveGraphic<Point2D?, Graphics2D?>?

    /** Point at end of arrow  */
    protected val end: PrimitiveGraphic<Point2D?, Graphics2D?>?

    /**
     * Adds the points to the composite graphic. If subclasses desire to change
     * the draw order, they should override this method.
     */
    protected open fun initGraphics() {
        addGraphic(start)
        addGraphic(end)
    }

    fun getStartGraphic(): PrimitiveGraphic<Point2D?, Graphics2D?>? {
        return start
    }

    fun getEndGraphic(): PrimitiveGraphic<Point2D?, Graphics2D?>? {
        return end
    }
    ...
    init {
        ...
        this.start = JGraphics.marker(OrientedPoint2D(start), style)
        this.end = JGraphics.marker(OrientedPoint2D(end), style)
        initGraphics()
        pointsUpdated()
    }
}