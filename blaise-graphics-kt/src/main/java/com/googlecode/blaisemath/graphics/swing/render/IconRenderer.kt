 ...
 */
class IconRenderer : Renderer<AnchoredIcon?, Graphics2D?> {
    override fun render(primitive: AnchoredIcon?, style: AttributeSet?, canvas: Graphics2D?) {
    ...
    override fun boundingBox(primitive: AnchoredIcon?, style: AttributeSet?, canvas: Graphics2D?): Rectangle2D? {
        val anchor = Styles.anchorOf(style, Anchor.NORTHWEST)
        return anchor.rectangleAnchoredAt(primitive, primitive.getIconWidth().toDouble(), primitive.getIconHeight().toDouble())
    }

    override fun contains(point: Point2D?, primitive: AnchoredIcon?, style: AttributeSet?, canvas: Graphics2D?): Boolean {
        return boundingBox(primitive, style, canvas).contains(point)
    }

    override fun intersects(rect: Rectangle2D?, primitive: AnchoredIcon?, style: AttributeSet?, canvas: Graphics2D?): Boolean {
        return boundingBox(primitive, style, canvas).intersects(rect)
    }

    companion object {
        private val INST: IconRenderer? = IconRenderer()
        fun getInstance(): Renderer<AnchoredIcon?, Graphics2D?>? {
            return INST
        }
    }
}